...
async function initialize_based_on_user_role() {
    ...
    const roles = await getLorebookEntries(lorebook).then(entries => entries
        .filter(entry => entry.comment.includes('user身份是') && entry.enabled)
        .map(entry => entry.comment.replace('user身份是', '')));
    const initialization = _.mergeWith({}, ...roles.map(entry => _.defaultTo(default_variables[entry], {})), (object_value, source_value) => {
        return _.sample([object_value, source_value]);
    });
    let message = getChatMessages(0)[0].message;
    message = message.replace(/(?<=\|初始化变量\|)(\s*)(?=<UpdateVariable>)/, Object.entries(initialization)
        .map(([key, value]) => `\n@${key}=${value}@`)
        .join('') + '\n');
