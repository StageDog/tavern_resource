...
export const Settings = z
  .object({
    ...
    above_dx_placeholder: z.string().default('{{ABOVE_DX}}'),
    below_dx_placeholder: z.string().default('{{BELOW_DX}}'),
    stop_string: z.string().default('<|im_end|>').catch('<|im_end|>'),
  })
  .transform(data => {
    if (data.put_system_injection_after_chat_history) {
      data.move_above_dx_to_front = true;
      data.move_below_dx_to_back = true;
      data.put_system_injection_after_chat_history = false;
    }
    return data;
  });
