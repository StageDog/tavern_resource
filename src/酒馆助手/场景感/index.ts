    ...
    const view = await query_prompt_from_buttons('请选择视角', {
      ...
      选择媒体: async () => {
        const options = {
          ...
          正经严肃:
            '正经严肃（以客观中立的态度讨论各类话题，重视事实依据，避免戏谑和轻浮，展现出专业和权威的氛围）类型',
          ...
          色情淫靡:
            '色情淫靡（将女性视为泄欲工具，口不遮掩地说肉便器、鸡吧套子等淫语，对女性进行意淫和侮辱，并幻想女性和{{user}}的关系）类型',
          痴女发癫:
            '痴女发癫（都是爱慕{{user}}的痴女，互相之间帮助或雌竞争吵，毫不掩饰地表达对{{user}}各种想法（包括变态、淫乱的想法），最终目的是攻略{{user}}，必须有主要角色在上面偷偷发言）类型',
        } as const;
